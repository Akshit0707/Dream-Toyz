...
export async function getCarFilters() {
    try {
        ...
        const priceAgreegations = await db.car.aggregate({
            where: { status: "AVAILABLE" },
            _min: { price: true },
            _max: { price: true },
        });
        ...
        return {
            ...
            data: {
                ...
                priceRange: {
                    min: 400000,   // your new min price
                    max: 100000000 // your new max price
                }
        ...
        throw new Error("Error Fetching Car Filter" + error.message);
    }
}
...
export async function getCars({
    ...
    try {
        ...
        if (maxPrice && maxPrice < Number.MAX_SAFE_INTEGER) { where.price.lte = parseFloat(maxPrice); };
        ...
        const serializeCars = cars.map((car) =>
...
export async function toogleSavedCars(carId) {
    try {
        const { userId } = await auth();
        ...
        throw new Error("Error toogling saved car" + error.message);
