...
import { LogOut, Car, Calendar, Cog, LayoutDashboard } from "lucide-react";
...
const Sidebar = () => {
  ...
  return (
    <>
      {/* DESKTOP Sidebar */}
      <div className="hidden md:flex flex-col w-56 h-[calc(100vh-80px)] fixed top-20 left-0 bg-white shadow-sm border-r">
        <div className="p-6">
          <Link href="/admin">
      ...
      <div className="flex md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t justify-around items-center h-16 shadow-lg">
        {routes.map((route) => (
          <Link
            key={route.href}
            href={route.href}
